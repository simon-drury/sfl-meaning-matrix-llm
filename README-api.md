...
[parser]              <- input edge
     |
     v
MeaningTrajectory     <- transformer sees only this (9D states)
     |
     v
...
```json
{
  ...
  "lang_out": "EN",
  "trajectory": [
    {"t": 0, "state": [-0.7, -0.5, 0.8, 0.9, 0.6, -0.6, 0.3, 0.2, -0.4], "label": "hey"},
    {"t": 1, "state": [-0.6, -0.3, 0.9, 0.9, 0.8, -0.6, 0.3, 0.2, -0.4], "delta": 0.26, "kappa": null, "phi": "interpersonal×tenor"}
  ],
  "path_length": 1.43,
  "realization": {
    "modality": "text",
...
curl -X POST http://127.0.0.1:8000/realize \
  -d '{"M_out": [0.1, 0.6, 1.0, 1.0, 0.8, -0.6, 0.3, 0.2, -0.1], "modality": "text", "lang": "EN"}'
# -> "thank you"

# ES realization — same M_out
curl -X POST http://127.0.0.1:8000/realize \
  -d '{"M_out": [0.1, 0.6, 1.0, 1.0, 0.8, -0.6, 0.3, 0.2, -0.1], "modality": "text", "lang": "ES"}'
# -> "gracias"
```
...
```python
REALIZERS["audio:EN"] = AudioRealizerEN()   # same .nearest() interface
REALIZERS["visual"]   = VisualRealizer()
```
