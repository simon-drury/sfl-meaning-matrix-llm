...
# Default empirical centroid mapping for Voronoi realization
DEFAULT_LEXICON = {
...
def prompt_to_matrix(prompt, dim=9):
    # Deterministic seed from prompt hash to initialize M_0 on the semiotic manifold
    np.random.seed(abs(hash(prompt)) % (2**32))
    m0 = np.random.uniform(0.1, 0.9, size=dim).astype(np.float32)
    return m0

def realize_step(matrix_vec, lexicon=DEFAULT_LEXICON, top_k=3):
    # Voronoi nearest-neighbor cell lookup
    dists = {}
    for word, centroid in lexicon.items():
        if len(centroid) != len(matrix_vec):
            ...
            c = centroid
        dists[word] = np.linalg.norm(matrix_vec - c)
...
def run_inference(prompt, model_path="sfl_model_3x3.pt", steps=5, dim=9):
    ...
    if os.path.exists(model_path):
        try:
            ...
            print(f"[Model] Warning: Could not load exact state_dict ({e}). Using initialized manifold transformer.")
    else:
        print(f"[Model] {model_path} not found. Running with geometric manifold prior.")
    ...
    for s in range(steps):
        ...
        # Advance state along predicted geodesic delta
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, default="Adam Smith division of labour and wealth of nations", help="Input prompt")
    ...
    run_inference(args.prompt, model_path=args.model_path, steps=args.steps, dim=args.dim)
